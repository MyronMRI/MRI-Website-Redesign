        ...
        <div class="body_main">
            ...
            <div class="body_page">
				...
				<div class="body_page_main">
					...
					<?php $counter = 0 ?>
					<?php $skipcounter = 0 ?>
					<span style="color: #494b14;font-size: 15px;line-height: 14px;font-weight: bold;margin-bottom: 20px;display: block;">Archive: <?php if ($_GET["month"] != $_GET["year"]){ ?><?php echo $_GET["month"]; ?> <?php } ?><?php echo $_GET["year"]; ?></span>
					<?php foreach ($blog_entriesRecords as $record): ?>
						   <?php if ($_GET['month'] != $_GET['year']): ?>
                        		<?php if ((date("F", strtotime($record['post_date']))) != ($_GET['month']) ) { continue; } ?>
							<?php endif ?>
                        <?php if ((date("Y", strtotime($record['post_date']))) != ($_GET['year']) ) { continue; } ?>
						<?php $skipcounter++ ?>
					...
					<?php endforeach ?>
						<?php if ($counter <= 0){ ?>
							There are no posts from <?php echo $_GET["month"]; ?> <?php echo $_GET["year"]; ?> in the archive. Please select a different date or <a href="/blog/">click here</a> to return to the blog home page.
							<?php } ?>
                        ...
                        <?php if ((date("Y", strtotime($record['post_date']))) != ($_GET['year']) ) { continue; } ?>
							...
							<?php if($prev >= 1){ ?>
								<a href="archive.php?month=<?php echo$_GET['month'] ?>&year=<?php echo$_GET['year'] ?>&pg=<?php echo '1' ?>"><img src="nav_arrows/arrow_orange_first.png" height="10" /></a>
							<?php }else{ ?> 
								<img src="nav_arrows/arrow_off_first.png" height="10" />
							<?php } ?>
							<?php if($prev >= 1){ ?>
								<a href="archive.php?month=<?php echo$_GET['month'] ?>&year=<?php echo$_GET['year'] ?>&pg=<?php echo $prev ?>"><img src="nav_arrows/arrow_orange_l.png" height="10" /></a> 
							<?php }else{ ?> 
							...
							<?php if($next <= $pagecounter){ ?>
								<a href="archive.php?month=<?php echo$_GET['month'] ?>&year=<?php echo$_GET['year'] ?>&pg=<?php echo $next ?>"><img src="nav_arrows/arrow_orange.png" height="10" /></a>
							<?php }else{ ?> 
								<img src="nav_arrows/arrow_off.png" height="10" />
							<?php } ?>
							<?php if($next <= $pagecounter){ ?>
								<a href="archive.php?month=<?php echo$_GET['month'] ?>&year=<?php echo$_GET['year'] ?>&pg=<?php echo $pagecounter ?>"><img src="nav_arrows/arrow_orange_last.png" height="10" /></a>
							<?php }else{ ?> 
