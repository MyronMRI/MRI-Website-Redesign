...
<div id="container">
    ...
    <div id="body">
		...
		<?php foreach ($around_the_worldRecords as $atwhead): ?>
			<?php $query = $_GET["city"] ?>
			<?php if ($query != $atwhead['title']){continue;} ?>
            ...
            <div class="body_page">
				...
				<div class="body_page_main">
					...
					<?php foreach ($blog_entriesRecords as $record): ?>
			 			<?php if ($record['category:label'] != 'Around the World'){continue;} ?>
						<?php $query = $_GET["city"] ?>
						<?php $querystr = str_replace("+", " ", $query); ?>
						<?php if (($querystr != $record['city:label'])) {continue;} ?>
						<?php $skipcounter++ ?>
					...
					<?php endforeach ?>
						...
						<div id="pagination" style="margin:10px auto;text-align:center;">
			 			...
			 			<?php if ($record['category:label'] != 'Around the World'){continue;} ?>
						<?php $query = $_GET["city"] ?>
						<?php $querystr = str_replace("%20", " ", $query); ?>
						<?php if (($querystr != $record['city:label'])) {continue;} ?>
						<?php $pagecounter ++;} ?>
