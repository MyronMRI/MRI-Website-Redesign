        ...
        <div class="body_main">
			 ...
			 <?php if(!empty($_GET["category"])){?>
				<?php foreach ($blog_categoriesRecords as $categoryhead): ?>
					...
					<?php if($categoryhead['header_photo']): ?>
						<img src="<?php foreach ($categoryhead['header_photo'] as $index => $upload): ?><?php echo $upload['urlPath'] ?><?php endforeach ?>"/>
					<?php endif ?>
				<?php endforeach ?>	
				<?php } else if(!empty($_GET["city"])){	?>
					<?php foreach ($around_the_worldRecords as $atwhead): ?>
					<?php $querystr = str_replace("+", " ", $_GET["city"]) ?>
					<?php if (($querystr != $atwhead['title'])){continue;} ?>
						<img src="<?php foreach ($atwhead['header_photo'] as $index => $upload): ?><?php echo $upload['urlPath'] ?><?php endforeach ?>">
					<?php endforeach ?>
				<?php } else { ?>
				<img src="<?php foreach ($blog_homepageRecord['header_photo'] as $upload): ?><?php echo $upload['urlPath'] ?><?php endforeach ?>" />
				<?php } ?>
            ...
            <div class="body_page">
				...
				<div class="body_page_main">
					...
					<span style="color: #494b14;font-size: 15px;line-height: 14px;font-weight: bold;margin-bottom: 20px;display: block;">City: <?php echo str_replace("+", " ", $_GET["city"]); ?></span>
					<?php } else if(!empty($_GET["post_date_year"])){	?>		
					<span style="color: #494b14;font-size: 15px;line-height: 14px;font-weight: bold;margin-bottom: 20px;display: block;">Posts Dated <?php echo date("F", mktime(0, 0, 0, $_GET["post_date_month"], 10)) . " " . $_GET["post_date_year"]; ?></span>
					<?php } ?>
