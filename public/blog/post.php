<?php
$redirectlink = 'http://www.marketingresults.net/mobile/blog/post.php?title=' . @$_GET['title'];
...
if (is_numeric(@$_GET['title'][strlen($_GET['title'])-1]) && (substr($_GET['title'], -2, 1) == ' ' || substr($_GET['title'], -3, 1) == ' ' || substr($_GET['title'], -4, 1) == ' ')) {
  	$accessedByPermalink = false;
...
<?php if($accessedByPermalink): ?>
<meta property="og:url" content="http://www.marketingresults.net<?php echo $blog_entriesRecord['_link']; ?>" />
<?php else: ?>
			 ...
			 <div class="blog_header" style="height:112px;background-image:url(
			 	<?php foreach ($blog_entriesRecords as $record): ?>
					...
					<?php if ($categoryhead['parent_category']){ ?>
						/blog/headers/<?php foreach ($blog_categories_newRecords as $colors){if ($colors['title'] == $categoryhead['parent_category']){echo $colors['category_color'];}} ?>.jpg
					<?php } else if ($categoryhead['category_color']){ ?>
						headers/<?php echo $categoryhead['category_color']; ?>.jpg
					<?php } else { ?>
						/blog/headers/blue.jpg
			 		<?php } endforeach ?>
				<?php endforeach ?>
				<?php if(count($blog_entriesRecords) <= 0) {echo "/blog/headers/blue.jpg";} ?>
			 )">
			 <?php foreach ($blog_entriesRecords as $record): ?>
				<?php foreach ($blog_categories_newRecords as $categoryhead): ?>
					<?php if ($record["categories"] != $categoryhead['title']){continue;} ?>
				...
				<?php endforeach ?>
			 <?php endforeach?>
			 <?php if(count($blog_entriesRecords) <= 0) { ?>
			 		<span class="header_headline">Marketing Results Blog</span>
			 <?php } ?>
			 </div>
            <div class="body_page" style="background: #f8eeda;background-image:url(/blog/bg.jpg);background-repeat:repeat-x;background-position:top;">
				...
				<div class="body_page_main">
					...
					<span style="color: #953108;font-size: 18px;line-height: 14px;margin-bottom: 24px;display: block;letter-spacing: 2px;text-transform: uppercase;margin-top:-4px;">					
						<?php foreach ($blog_entriesRecords as $record): ?>
							...
							<?php endforeach ?>
						<?php } endforeach ?>
						<?php if(count($blog_entriesRecords) <= 0) {echo "We're Sorry";} ?>
					</span>
					<?php if(count($blog_entriesRecords) <= 0) { ?>
					<div class="blog_post index_post">
					<div style="margin-top: 2px;padding-top: 8px;border-top: 1px solid #3b687b;">
					<p>
						We were unable to load the blog post you requested. You can use the search functionality in the right-hand column or <a href="http://www.marketingresults.net/blog/">visit the blog home page</a> to view all posts.
					</p>
					</div>
					</div>
					<?php } ?>
					...
					<div class="blog_post index_post">
						...
						in
						<a href="index.php?categories=<?php echo str_replace(" ", "+", $record['categories:label']) ?>" style="text-decoration:underline !important;"><?php echo str_replace('®', '<sup class="cat-title">&reg;</sup>', $record['categories:label']) ?></a>
						<span style=""><a href="post.php?title=<?php echo str_replace(" ", "+", $record['title']) . '+' . $record['num'] ?>#disqus_thread" data-disqus-identifier="ident<?php echo $record['num'] ?><?php echo preg_replace("/[^A-Za-z0-9]/","",$record["title"]) ?>" style="float:right;">Comments</a></span>
						<br />
						<div style="margin-top: 2px;padding-top: 8px;border-top: 1px solid #3b687b;">
					...
					<div class="div"></div>
					  <div id="disqus_thread"></div>
						<script type="text/javascript">
							...
							var disqus_identifier = 'ident<?php foreach ($blog_entriesRecords as $record): ?><?php echo $record["num"] ?><?php echo preg_replace("/[^A-Za-z0-9]/","",$record["title"]) ?><?php endforeach ?>';
							var disqus_url = 'http://www.marketingresults.net/blog/post.php?title=<?php echo str_replace(" ", "+", $record["title"]) . '+' . $record['num'] ?>';
